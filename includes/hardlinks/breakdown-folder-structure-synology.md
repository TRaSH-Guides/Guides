...
Sonarr, Radarr, Readarr and Lidarr get access to everything using `/data` because the download and media folder(s) will look like and be one file system. Hardlinks will work and moves will be atomic (*instead of copy + delete*).
...
Plex, Emby, JellyFin and Bazarr only need access to your media library using `/data/media`, which can have any number of sub folders like Movies, Kids Movies, TV, Documentary TV, or Music as subfolders.
