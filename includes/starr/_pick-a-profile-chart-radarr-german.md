...
flowchart TD
    START([Start: pick a [German] Radarr profile]) --> KIND{Content kind?}
    KIND -->|German live-action movies| RES{Display + player<br/>resolution?}
    KIND -->|German anime movies| P_ANIME(["[German] Anime HD Bluray + WEB"])

    RES -->|1080p only| HD_SRC{Source<br/>preference?}
    HD_SRC -->|Encode| P_DE_HD(["[German] HD Bluray + WEB"])
    ...
    UHD_SRC -->|Remux| P_DE_UHDR(["[German] Remux + WEB 2160p"])
