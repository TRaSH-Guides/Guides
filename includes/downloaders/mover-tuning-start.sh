...
check_config_version() {
    ...
    if command -v curl &> /dev/null; then
        ...
        log "⚠ Cannot check config version: curl or wget not found"
        return 0
    fi

    # Fetch the latest config from GitHub
    local remote_config
    remote_config=$($fetch_cmd "$CONFIG_RAW_URL" 2>/dev/null)

    if [[ -z "$remote_config" ]]; then
        log "⚠ Could not fetch latest config from GitHub"
        return 0
    fi

    # Extract version from the remote config
    local remote_config_version
    remote_config_version=$(echo "$remote_config" | grep -m1 "^readonly CONFIG_VERSION=" | sed 's/readonly CONFIG_VERSION="\([^"]*\)".*/\1/')

    if [[ -z "$remote_config_version" ]]; then
        log "⚠ Could not parse version from remote config (version may not be set yet)"
        return 0
    fi
    ...
    if [[ "$current_config_version" != "$remote_config_version" ]]; then
        # Simple version comparison (works for semantic versioning)
        if printf '%s\n' "$remote_config_version" "$current_config_version" | sort -V | head -n1 | grep -q "^$current_config_version$"; then
