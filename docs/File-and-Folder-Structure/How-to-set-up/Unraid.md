...
!!! tip "Create a custom Docker network"
    ...
    A major benefit of having your Docker containers on the same custom Docker network is that they will be able to communicate with other using their container names with the `.internal` domain (e.g., `radarr.internal`, `sonarr.internal`), rather than having to use IP addresses.
